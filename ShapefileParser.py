...
# %% Load Packages
from ShapefileParserFunctions import get_satellite_text_file, calculatesPixelSizeVIIRS, calculatesScanAngleMODIS, calculatesPixelSizeMODIS
import os
import numpy as np
...
shapefile_the_netherlands = geopandas.read_file(
    "C:\\Users\\Coen\\Documents\\Universiteit\\Earth Science msc 2019 - 2020\\Research Project\\Files\\shapefile_border_netherlands\\bordernetherlands.shp")
...
# max and min longitude of the Netherlands in degrees based on EPSG:28992
longitude = [3.2, 7.22]

# max and min latitude of the Netherlands in degrees based on EPSG:28992
latitude = [50.75, 53.7]
...
for filename in files:
    ...
    satellite_data = get_satellite_text_file(filepath=filename,
                                             delimiter=delimiter)

    # Filtering based on the location
    longitude_query = 'lon > ' + \
        str(longitude[0]) + ' and lon < ' + str(longitude[-1])
    latitude_query = 'lat > ' + \
        str(latitude[0]) + ' and lat < ' + str(latitude[-1])
    satellite_data.query(longitude_query + ' and ' +
                         latitude_query, inplace=True)

    # Filtering based on type
    # Type 0 = presumed vegetation data
    satellite_data.query('type == 0', inplace=True)
    ...
    satellite_data['geometry'] = satellite_data.apply(
    ...
    shapefile = geopandas.GeoDataFrame(
        satellite_data, geometry='geometry', crs=pyproj.CRS('EPSG:4326'))

    shapefile = shapefile.to_crs(pyproj.CRS('EPSG:28992'))

    joined_shapefile = geopandas.tools.sjoin(
        shapefile, shapefile_the_netherlands, how="left")
    ...
    if satellite == "VIIRS":
        ...
        pixel_sizes = calc.calculatesPixelSizeVIIRS(
            shapefile["sample"], pixel_nadir, row_measurements)

    elif satellite == "MODIS":
        row_measurements = 1354
        pixel_nadir = 1  # km
        satellite_altitude = 705  # km
        scan_angle = calc.calculatesScanAngleMODIS(
            amount_row_measurements=row_measurements, height=satellite_altitude, pixel_nadir=pixel_nadir, row_number=np.array(shapefile["sample"]))
        pixel_sizes = calc.calculatesPixelSizeMODIS(
            scan_angle, satellite_altitude, pixel_nadir)
    ...
    for index in range(len(shapefile.geometry)):
        leftupper_point = Point(geometry[index].x - pixel_sizes["delta_scanline"]
                                [index] / 2, geometry[index].y + pixel_sizes["delta_trackline"][index] / 2)
        rightdown_point = Point(geometry[index].x + pixel_sizes["delta_scanline"]
                                [index] / 2, geometry[index].y - pixel_sizes["delta_trackline"][index] / 2)
        new_geometry.append(MultiPoint(
            points=[leftupper_point, rightdown_point]).envelope)
    ...
    if final_shapefile is None:
        final_shapefile = geopandas.GeoDataFrame(
            shapefile, geometry=shapefile.geometry, crs=pyproj.CRS('EPSG:28992'))
    else:
        final_shapefile = final_shapefile.append(
            geopandas.GeoDataFrame(shapefile, geometry=shapefile.geometry))
