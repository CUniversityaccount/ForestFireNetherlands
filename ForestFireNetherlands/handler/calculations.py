...
"""
TODO:
- Zorgen dan VIIRS data wordt veranderd voor de delta S en delta T
Hierbij is van belang dat 750 naar 
"""
def calculatesScanAngleMODIS(amount_row_measurements, height, pixel_nadir, row_number):
    ...
    """
    scan_angle = abs((pixel_nadir / height) * (-0.5 * amount_row_measurements + 0.5 + (row_number - 1)))
    return scan_angle
...
def calculatesPixelSizeMODIS(scan_angle, satellite_height, pixel_nadir):
    ...
    delta_S = earth_radius * s * (np.cos(scan_angle) / np.sqrt(((earth_radius / r) ** 2) - (np.sin(scan_angle) ** 2)) - 1)
    delta_T = r * s * (np.cos(scan_angle) - np.sqrt((earth_radius / r) ** 2 - (np.sin(scan_angle) ** 2)))
